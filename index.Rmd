...
legend(5,11,legend=c("AD","non-AD"),cex=0.6,fill=c("turquoise","mediumvioletred"),xpd=TRUE)
arrows(bp.Hipo,t(dfHipo)+sdevHipo, bp.Hipo, t(dfHipo), angle=90, code=1,length = 0.15,xpd=TRUE)
```
**Figura 2 -** Comparação dos níveis de expressão de 12 genes cuja expressão diferencial é significativamente diferente no hipocampo AD em relação a non-AD.
...
```
**Figura 3 -** Comparação dos níveis de expressão de 8 genes cuja expressão diferencial é significativamente diferente no córtex temporal AD em relação a non-AD.
...
FALTA O BARPLOT
```{r, message=FALSE, warning=FALSE}
ssAD = subAD[c(rownames(upAD),rownames(upAD)),]
```
**Figura 4 -** Comparação dos níveis de expressão de 8 genes cuja expressão diferencial é significativamente diferente no género feminino em relação ao masculino, na AD. 
...
```  
**Figura 4 -** _Clustering_ das amostras referentes aos genes diferencialmente expressos no hipocampo. 
...
```
**Figura 5 -** _Heatmap_ dos dados de expressão do hipocampo.
...
legend('topright', c("non-AD","AD"), bty = "n", fill = c(4, 3))
```
**Figura 6 -** _Clustering_ das amostras referentes aos genes diferencialmente expressos no córtex temporal. 
...
legend("topright",legend=c("AD","non-AD"), bty="n",fill = c("turquoise","chocolate1"))
```
**Figura 7 -** _Heatmap_ dos dados de expressão do córtex temporal.

Analisando o _heatmap_, verifica-se que as amostras não são totalmente separadas, tal como no _cluster_ anterior. (Falta aqui o resto da interpretação, tenho de ver o heatmap antes de escrever)
