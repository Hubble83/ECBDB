...
author:
- name: "Beatriz Magalhães PG32936"
- name: "Cláudio Nóvoa PG32942"
- name: "João Afonso A71874"
- name: "Marta Sampaio PG32927"
...
A filtração pela mediana corresponde à seleção dos dados cujo desvio padrão se situa à direita da linha da mediana do histograma.
...
Na análise de expressão diferencial usou-se o Expression Set completo, contudo na identificação de genes siginificativamente sobreexpressos e subexpressos analisaram-se apenas os genes anotados, com _p-value_ < 0.05 e _Fold Change_ > _treshold_, para estar de acordo com a análise realizada no artigo. 
...
nomes_upHipo
```
No hipocampo, 3 dos 83 genes estão sobreexpressos na Doença de Alzheimer. 
...
#### **Análise de Enriquecimento para os genes sobreexpressos**
...
nomes_upTemp
```
No Córtex Temporal, 5 dos 8 genes estão sobreexpressos na Doença de Alzheimer. 
...
namesAD.up
```
7 dos 9 genes estão sobreexpressos no género feminino. 
...
Os genes sobreepressos no sexo feminino desempenham funções como regulação negativa da resposta inflamatória crónica, sinalização de recetores acoplados à proteína G, regulação da potenciação sináptica a longo-prazo, entre outras. A relação entre todas estas funções e a doença de Alzheimer já foi previamente explicada.
...
Os genes subexpressos no sexo feminino estão envolvidos na demetilação de histonas, o que provoca a quebra dos nucleossomas de DNA, que, em condições atípicas provoca uma desregulação da transcrição [4]. Relacionando com AD, isto pode levar a um mal-funcionamento dos neurónios e da comunicação entre estes, podendo eventualmente, ser uma das causas da sua destruição. 
...
Analisando o _heatmap_, verifica-se uma evidente separação das amostras, tal como no _cluster_ anterior. Para além disso, observa-se vários agrupamentos dos genes, que parecem estar agrupados consoante os seus níveis de expressão, já que se percebe uma grande porção com uma côr mais intensa (genes sobreexpressos) à esquerda do gráfico pertecente às amostras AD e uma porção com côr menos intensa (genes subexpressos) abaixo da anterior.
...
Analisando o heatmap, verifica-se que as amostras não são totalmente separadas, tal como no cluster anterior, e que apesar de haver alguma diferença de intensidade de intensidade de côr entre as caixas correspodentes a AD e non-AD, a separação não é tão distinta como no caso do hipocampo.
...
Ainda, verificamos que o hipocampo é o tecido com maior expressão diferencial de genes entre AD e non-AD, corroborando a conclusão do artigo analisado, e que, como esperado, existe diferença na expressão entre sexo feminino e masculino, o que pode explicar a diferente incidência encontrada nos dois sexos.
